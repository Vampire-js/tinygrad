...
public:
  ...
  void reshape(std::vector<std::size_t> shape){
    ...
    for(int i: shape) expected *= i;
    if(expected == numel() && shape.size() == rank()) shape_ = std::move(shape);
    else throw std::invalid_argument("Incorrect shape");
}
    void transpose(){
...
/*
TODO:
- Overloading operators for calculations
- GPU compute for tensor calculations
*/
...
int main() {
  ...
  std::cout << "Test 18\n";
  assert((Tensor({4}, {1,2,3,4})*Tensor({4}, {0,1,1,1})).sum() == 9);
